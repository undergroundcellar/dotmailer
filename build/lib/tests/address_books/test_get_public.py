...
def test_get_public(sample_address_book, sample_public_address_book):

    address_books = AddressBook.get_all(address_book_type='Public')
    for book in address_books:
        print(book.id, book.name, book.visibility)
    assert sample_address_book not in address_books
    assert sample_public_address_book in address_books


def test_get_public_limit(request, connection):
    ...
    existing_books = AddressBook.get_all(address_book_type='Public')
